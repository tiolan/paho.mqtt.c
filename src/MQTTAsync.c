...
int MQTTAsync_connect(MQTTAsync handle, const MQTTAsync_connectOptions* options)
{
	...
	if (m->c->sslopts)
	{
		...
		{
			if (m->c->sslopts->privateKeyString)
				free((void*)m->c->sslopts->privateKeyString);
			if (m->c->sslopts->clientCertString)
				free((void*)m->c->sslopts->clientCertString);
		}
	...
	if (options->struct_version != 0 && options->ssl)
	{
		...
		if (m->c->sslopts->struct_version >= 6)
		{
			if (options->ssl->privateKeyString)
				m->c->sslopts->privateKeyString = MQTTStrdup(options->ssl->privateKeyString);
			if (options->ssl->clientCertString)
				m->c->sslopts->clientCertString = MQTTStrdup(options->ssl->clientCertString);
		}
	}
#else
	if (options->struct_version != 0 && options->ssl)
